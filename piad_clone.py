...
logo =f"""{A} 
            ...
            ╚═╝░░╚═╝╚═╝╚═╝░░╚═╝╚═╝
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} 𝙾𝚠𝚗𝚎𝚛     {A}:{G1} 🄰🅉🄸🅉🅄🄻 🄸🅂🄻🄰🄼 
{G1}[{A}={G3}]{G3} 𝚃𝚘𝚘𝚕 𝚃𝚢𝚙𝚎 {A}:{G3} 𝙵𝚒𝚕𝚎 {A}&{G4} 𝚁𝚊𝚗𝚍𝚘𝚖 𝚌𝚕𝚘𝚗𝚒𝚗𝚐 
{G1}[{A}={G4}]{G4} 𝚂𝚃𝙰𝚃𝚄𝚂    {A}:{G4} 𝚂𝙲𝚁𝙸𝙿𝚃 𝙵𝙾𝚁 𝙿𝙰𝙸𝙳
{G1}[{A}={G5}]{G5} 𝚅𝚎𝚛𝚜𝚒𝚘𝚗  {A}:{G5} ➊.➎
{A}──────────────────────────────────────────────────"""
...
def menu():   
    clear()
    print(f'{G1}[{A}1{G1}]{G1} 𝙁𝙄𝙇𝙀 𝘾𝙇𝙊𝙉𝙄𝙉𝙂')
    print(f'{G1}[{A}2{G2}]{G2} 𝙍𝘼𝙉𝘿𝙊𝙈 𝘾𝙇𝙊𝙉𝙄𝙉𝙂')
    linex()
...
def _randm_():   
    ...
    print(f'{G1}[{A}0{G3}]{G3} 𝘽𝘼𝘾𝙆 𝙏𝙊 𝙈𝘼𝙄𝙉 𝙈𝙀𝙉𝙐.')
    linex()
    select = input(f'{G1}[{A}?{G5}]{G5} CHOICE {A}:{G5} ')
    if select =='1':
        _bd_()
...
def _bd_():
    ...
    print(f'{G1}[{A}={G2}]{G2} TOTAL 🅾🅺 🅸🅳  {A}:{G2} {str(len(ok))}')
    print(f'{G1}[{A}={G3}]{G3} TOTAL 🅲🅿 🅸🅳  {A}:{G3} {str(len(cp))}')
    print(f'\r{A}──────────────────────────────────────────────────')
...
def _file_():
    global methods
    clear()
    print(f'{G1}[{A}1{G1}]{G1} 𝙼𝙴𝚃𝙷𝙾𝙳 {G1}[{A}M1{G1}]{G1} ')
    print(f'{G1}[{A}2{G2}]{G2} 𝙼𝙴𝚃𝙷𝙾𝙳 {G2}[{A}M2{G2}]{G1} ')
    linex()
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}random{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
